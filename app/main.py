...
class Widget(Base):
    ...
    submissions: Mapped[list["Submission"]] = relationship(back_populates="widget")
...
def seed_tenant(session: Session):
    if not session.scalar(select(Tenant)):
        session.add(Tenant(id=str(uuid.uuid4()), name="Demo Tenant", token="demo-token-change-me"))
        session.commit()
...
async def geo_lookup(ip: str) -> tuple[str, str, str]:
    providers = [os.getenv("GEO_PROVIDER_A_URL"), os.getenv("GEO_PROVIDER_B_URL")]
    for url in providers:
        if not url:
            continue
        try:
            async with httpx.AsyncClient(timeout=2.0) as client:
                response = await client.get(url, params={"ip": ip})
...
@app.get("/")
def health():
    return {"service": "widget-platform", "status": "ok"}
...
def create_widget(data: WidgetIn, authorization: Optional[str] = Header(None), session: Session = Depends(db)):
    ...
    widget = Widget(id=str(uuid.uuid4()), tenant_id=tenant.id, name=data.name, title=data.title, fields_json=json.dumps(data.fields))
...
def list_widgets(authorization: Optional[str] = Header(None), session: Session = Depends(db)):
    tenant = require_tenant(authorization, session)
    widgets = session.scalars(select(Widget).where(Widget.tenant_id == tenant.id)).all()
    return [{"id": w.id, "name": w.name, "title": w.title, "fields": json.loads(w.fields_json)} for w in widgets]
...
def update_widget(widget_id: str, data: WidgetIn, authorization: Optional[str] = Header(None), session: Session = Depends(db)):
    ...
    widget.name, widget.title, widget.fields_json = data.name, data.title, json.dumps(data.fields)
    session.commit()
...
def delete_widget(widget_id: str, authorization: Optional[str] = Header(None), session: Session = Depends(db)):
    tenant = require_tenant(authorization, session); widget = widget_for_owner(widget_id, tenant, session)
    session.delete(widget); session.commit(); return Response(status_code=204)
...
def embed_snippet(widget_id: str, authorization: Optional[str] = Header(None), session: Session = Depends(db)):
    tenant = require_tenant(authorization, session); widget_for_owner(widget_id, tenant, session)
...
def owner_submissions(widget_id: str, authorization: Optional[str] = Header(None), session: Session = Depends(db)):
    tenant = require_tenant(authorization, session); widget_for_owner(widget_id, tenant, session)
    rows = session.scalars(select(Submission).where(Submission.widget_id == widget_id).order_by(Submission.created_at.desc())).all()
...
async def public_submit(widget_id: str, data: SubmissionIn, request: Request, session: Session = Depends(db)):
    ...
    if not data.email and not data.message:
        raise HTTPException(422, "At least email or message is required")
    country, city, geo_status = await geo_lookup(ip)
    submission = Submission(
        id=str(uuid.uuid4()), widget_id=widget_id, name=data.name, email=data.email,
        message=data.message, ip=ip, country=country, city=city, geo_status=geo_status,
    )
    session.add(submission); session.commit(); session.refresh(submission)
    submission.side_effect_status = await send_side_effects(submission)
    session.commit()
...
def widget_script(id: str):
    if not id:
        raise HTTPException(400, "Widget id is required")
    base = os.getenv("PUBLIC_BASE_URL", "http://127.0.0.1:8000")
    js = f"""(function(){{\n  const id={json.dumps(id)}; const base={json.dumps(base)};\n  const root=document.currentScript.parentElement || document.body;\n  const box=document.createElement('div'); box.style.cssText='font-family:Arial;max-width:420px;padding:16px;border:1px solid #ddd;border-radius:10px';\n  box.innerHTML='<strong>Get in touch</strong><form><input name=\"name\" placeholder=\"Name\" style=\"display:block;width:100%;margin:8px 0;padding:8px\"><input name=\"email\" placeholder=\"Email\" style=\"display:block;width:100%;margin:8px 0;padding:8px\"><textarea name=\"message\" placeholder=\"Message\" style=\"display:block;width:100%;margin:8px 0;padding:8px\"></textarea><input name=\"website\" tabindex=\"-1\" autocomplete=\"off\" style=\"position:absolute;left:-9999px\"><button>Send</button><span data-status style=\"margin-left:8px\"></span></form>';\n  root.appendChild(box); const form=box.querySelector('form'); form.onsubmit=async(e)=>{{e.preventDefault();const body=Object.fromEntries(new FormData(form));const r=await fetch(base+'/api/public/widgets/'+id+'/submissions',{{method:'POST',headers:{{'Content-Type':'application/json'}},body:JSON.stringify(body)}});box.querySelector('[data-status]').textContent=r.ok?'Thanks!':'Please try again.';}};\n}})();"""
...
def dashboard_summary(authorization: Optional[str] = Header(None), session: Session = Depends(db)):
    tenant = require_tenant(authorization, session)
    total = session.scalar(select(func.count(Submission.id)).join(Widget).where(Widget.tenant_id == tenant.id)) or 0
    widgets = session.scalar(select(func.count(Widget.id)).where(Widget.tenant_id == tenant.id)) or 0
...
def widget_stats(widget_id: str, authorization: Optional[str] = Header(None), session: Session = Depends(db)):
    tenant = require_tenant(authorization, session); widget_for_owner(widget_id, tenant, session)
    total = session.scalar(select(func.count(Submission.id)).where(Submission.widget_id == widget_id)) or 0
...
def geo_stats(authorization: Optional[str] = Header(None), session: Session = Depends(db)):
    ...
    rows = session.execute(
        select(Submission.country, func.count(Submission.id)).join(Widget).where(Widget.tenant_id == tenant.id).group_by(Submission.country)
    ).all()
